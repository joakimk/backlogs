class Item < ActiveRecord::Base
  ...
  def self.find_by_project(project, hide_closed = false, main_backlog = nil)
    find(:all, :include => [ :issue, :backlog ],
               :conditions => "issues.project_id=#{project.id}" +
                              " AND items.parent_id=0 #{"AND (backlogs.is_closed = false OR backlogs.id = #{main_backlog.id})" if hide_closed}",
